...
local base_damage = 3000  -- Base damage for the explosion
...
while true do
  ...
  if technology.effects then
    for _, effect in ipairs(technology.effects) do
      if effect.type == "ammo-damage" and effect.ammo_category == "cannon-shell" and effect.modifier then
        -- Get the modifier (scale factor) for the technology
        local damage_multiplier = effect.modifier

        -- Apply the multiplier to the base damage
        local scaled_damage = base_damage * (1 + damage_multiplier)

        -- Update the explosion's damage
        data.raw["explosion"]["rc-car-explosion"].created_effect.action_delivery.target_effects[1].action.action_delivery.target_effects[1].damage.amount = scaled_damage
      end
    end
  end
